...
def CaptureVimCommand( command ):
  return vim.eval( f"execute( '{ EscapeForVim( command ) }' )" )


def UnplaceSignsInBuffer( bufnr ):
  vim.eval( f'sign_unplace( "YouCompleteMe", {{ "buffer": { bufnr } }} )' )


def PlaceSign( name, buffer_name, line ):
  vim.eval( f'sign_place( 0, "YouCompleteMe", "{ name }", '
            f'"{ buffer_name }", {{ "lnum": { line } }} )' )
